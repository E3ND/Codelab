...
export const ModuleList = ({ modules }: ModulesListProps) => {
    ...
    return (
        ...
        >
            <div onClick={handleToggleCollapsed} className={cn("absolute z-10 left-0 top-0 bottom-0 w-4 flex justify-start group cursor-e-resize group", modulesListCollapsed && "cursor-w-resize")}>
            ...
            {modulesListCollapsed ? (
                ...
                <>
                    <Button onClick={handleToggleCollapsed} variant="outline" className="w-full flex sm:hidden mb-4">Fechar módulo</Button>
                    <Accordion.Root 
                        type="single"
                        className="w-full h-full flex flex-col gap-3"
                        collapsible
                        defaultValue={moduleId}
                        value={expandedModue ?? undefined}
