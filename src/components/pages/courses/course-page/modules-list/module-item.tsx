...
export const ModuleItem = ({ data, completedLessons }: ModuleItemProps) => {
    ...
    const formattedDuraion = formatDuration(totalDuration);

    const lessons = useMemo(() => {
        return data.lessons.map((lesson) => {
            const completed = completedLessons.some((l) => l.lessonId === lesson.id);
    ...
    const moduleProgress = useMemo(() => {
        ...
        return (completedModuleLessons / totalLesson) * 100;
    }, [lessons, totalLesson])
    ...
    return (
        <Accordion.Item value={data.id} className="border border-border rounded-lg group">
            <Accordion.Trigger className="flex items-center gap-4 p-4 w-full hover:bg-muted/50 transition-all outline-none">
                <div className={cn(
                    "w-10 h-10 min-w-10 rounded-full flex items-center justify-center font-semibold bg-black/70 transition-all relative",
                    moduleProgress >= 100 && "text-primary bg-primary/10"
                )}>
                    {data.order}
                    <CircularProgress progress={moduleProgress} className="absolute inset-0 w-full h-full" />
                </div>
                ...
                <div className="flex-1 flex flex-col gap-0.5 text-left text-muted-foreground">
                    ...
                    <div className="flex items-center gap-6 text-xs">
                        <span>{totalLesson} aula{totalLesson > 1 ? "s" : ""}</span>
                        <span>{formattedDuraion}</span>
                    </div>
                </div>
